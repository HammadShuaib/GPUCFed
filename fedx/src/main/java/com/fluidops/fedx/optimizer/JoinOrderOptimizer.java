 ...
 * 
 * Additional Heuristics: - ExclusiveGroups are cheaper than any other subquery
...
//	public static Logger log = LoggerFactory.getLogger(JoinOrderOptimizer.class);
	...
	public static List<TupleExpr> optimizeJoinOrder(List<TupleExpr> joinArgs) {
		System.out.println("These are in optimizeJoinOrder:" + joinArgs);
		...
		System.out.println("These are in optimizeJoinOrder:" + optimized);
		return optimized;
	}
...
//ForkJoinPool fjp =new ForkJoinPool(6);
	...
	protected static double estimateCost(ExclusiveGroup group, Set<String> joinVars) {
		...
		// heuristic: a group has a selective statement, if one statement has 0 or 1
		// free variables
...
//			additionalCost += 1;
	...
	/**
	 * If the subject is not the same for all triple patterns in the group, an
	 * additional cost of .5 is considered.
	 * 
	 * Example: ?x p o . ?x p2 o2 => cost is 0 ?x p ?s . ?s ?p2 val => additional
	 * cost is 0.5
	 * 
	 * @return
	 */
	...
	protected static double estimateCost(ExclusiveStatement owned, Set<String> joinVars) {
		...
		// special heuristic: if exclusive statement with one free variable, more
		// selective than owned group
