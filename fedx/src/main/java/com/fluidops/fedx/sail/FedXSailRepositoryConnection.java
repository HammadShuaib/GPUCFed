...
public class FedXSailRepositoryConnection extends SailRepositoryConnection {
	...
	/**
	 * We add a binding to each parsed query mapping the original query in order to
	 * send the original query to the endpoint if there is only a single federation
	 * member is relevant for this query.
	 */
	...
	public FedXConnection getSailConnection() {
		System.out.println("This is timing in getSailConnection:"+LocalTime.now());
		
		ForkJoinPool fjp = new ForkJoinPool();
		try {
			return fjp.submit(()->(FedXConnection) super.getSailConnection()).get();
	...
	private void insertOriginalQueryString(SailQuery query, String queryString, QueryType qt) {
		query.setBinding(BINDING_ORIGINAL_QUERY, SimpleValueFactory.getInstance().createLiteral(queryString));
		query.setBinding(BINDING_ORIGINAL_QUERY_TYPE, SimpleValueFactory.getInstance().createLiteral(qt.name()));
	}
}
