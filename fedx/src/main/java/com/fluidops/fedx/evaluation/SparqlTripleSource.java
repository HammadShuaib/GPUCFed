...
public class SparqlTripleSource extends TripleSourceBase implements TripleSource {
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> getStatements(String preparedQuery,
		...
		System.out.println(
				"*************************************************This is in SparqlTripleSources1***********************************************************");
		...
		//	monitorRemoteRequest();
			res = query.evaluate();
			System.out.println("*************************************************This is in SparqlTripleSources3:" + res
					+ "--" + filterExpr);
			...
			if (filterExpr != null) {
				if (bindings.size() > 0) {
					...
					System.out
							.println("*************************************************This is in SparqlTripleSources4:"
									+ res + "--" + strategy + "--" + filterExpr);
			...
			Iterations.closeCloseable(res);
			throw ExceptionUtil.traceExceptionSourceAndRepair(strategy.getFedXConnection().getEndpointManager(), conn,
					ex, "Subquery: " + preparedQuery);
		}
	}
	...
	public boolean hasStatements(StatementPattern stmt, RepositoryConnection conn, BindingSet bindings)
		...
		if (useASKQueries) {
			...
			//	monitorRemoteRequest();
				...
				throw ExceptionUtil.traceExceptionSourceAndRepair(strategy.getFedXConnection().getEndpointManager(),
						conn, ex, "Subquery: " + queryString);
			}
	...
	@Override
	public boolean hasStatements(ExclusiveGroup group, RepositoryConnection conn, BindingSet bindings)
			throws RepositoryException, MalformedQueryException, QueryEvaluationException {
	...
	public CloseableIteration<Statement, QueryEvaluationException> getStatements(RepositoryConnection conn,
			...
			throws RepositoryException, MalformedQueryException, QueryEvaluationException {
		System.out.println(
				"*************************************************This is in SparqlTripleSources2***********************************************************");
