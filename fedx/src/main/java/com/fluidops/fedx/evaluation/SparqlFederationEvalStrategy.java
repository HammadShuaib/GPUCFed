...
//	ForkJoinPool fjp1 = new ForkJoinPool(6);
			Model model = //fjp1.submit(()->
			ModelFactory.createDefaultModel();//;).join();
			//fjp1.shutdown();
			//ForkJoinPool fjp2 = new ForkJoinPool(6);
			Dataset dataset = //fjp2.submit(()->
			DatasetFactory.create(model);//).join();
			//fjp2.shutdown();
			// initiate timer and results count for each query
		//	ForkJoinPool fjp3 = new ForkJoinPool(6);
			QueryExecution qe = //fjp3.submit(()->\
					QueryExecutionFactory.create(query1, dataset);//).join();
		//	fjp3.shutdown();
