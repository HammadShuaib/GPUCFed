 ...
 * Sail interface and can thus be used as a normal repository in a Sesame
 * environment. The federation layer enables transparent access to the
 * underlying members as if they were a central repository.
 * <p>
 * 
...
public class FedX implements Sail {
	...
	protected FedX(Config config, Cache cache, EndpointListProvider endpointListProvider,
			SummaryProvider summaryProvider) {
		System.out.println("This is timing in Fedx initialize:"+LocalTime.now());
...
//		this.statistics = statistics;
	...
	//	System.out
	//			.println("--------------------------------Inside FedX Constructor:------------------------------------"
	//					+ this.config + " " + this.cache + " " + this.statistics + " " + this.endpointListProvider + " "
...
//fjp.shutdown();
		...
		// httpClientBuilder.setConnectionReuseStrategy(new
...
httpClient = //fjp1.submit(()->
httpClientBuilder.build();//).join() ;
//fjp1.shutdown();
	//		if (monitoring == null) {
...
//				"---------------------------------------GetConfig in FedX--------------------------------------------------------"
//						+ config);
	//	log.debug("GetConfig now has: " + config);
		return config;
	}
...
//ForkJoinPool fjp = new ForkJoinPool();
		return //fjp.submit(()->
		new FedXConnection(this, endpointListProvider.getEndpoints(this), summaryProvider.getSummary(this));//).join();
	}
