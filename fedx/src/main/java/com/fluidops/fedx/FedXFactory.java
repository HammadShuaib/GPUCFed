...
//	protected static Logger log = LoggerFactory.getLogger(FedXFactory.class);
	...
	//	ForkJoinPool fjp = new ForkJoinPool();
		System.out.println("This is timing in initializeSparqlFederation first:"+LocalTime.now());
	System.out.println("This is sparqlendpoint:"+sparqlEndpoints);
		return //fjp.submit(()->
		initializeFederation(config, new DefaultEndpointListProvider(sparqlEndpoints),
					config.getSummaryProvider(), config.getCacheLocation());//).join();
	}
	...
	//	log.info("Loading federation members from dataConfig " + dataConfig + ".");
		return initializeFederation(config, null,
				config.getSummaryProvider(), config.getCacheLocation());
	}

	public static FedXSailRepository initializeFederation(Config config) throws Exception {
		return initializeFederation(config, config.getEndpointListProvider(), config.getSummaryProvider(),
				config.getCacheLocation());
	}

	/**
	 * Initialize the federation by providing information about the fedx
	 * configuration (c.f. {@link Config} for details on configuration parameters)
	 * and additional endpoints to add. The fedx configuration can provide
	 * information about the dataConfig to be used which may contain the default
	 * federation members.
	 * 
	 * The Federation employs a {@link MemoryCache} which is located at
	 * {@link Config#getCacheLocation()}.
	 * 
	 * @param fedxConfig          the location of the fedx configuration
	 * @param additionalEndpoints additional endpoints to be added, may be null or
	 *                            empty
	 * 
	 * @return the initialized FedX federation {@link Sail} wrapped in a
	 *         {@link SailRepository}
	 ...
	 * @throws Exception
	 */
	public static FedXSailRepository initializeFederation(String fedxConfig, EndpointListProvider endpointListProvider)
		...
		return initializeFederation(config, endpointListProvider, config.getSummaryProvider(),
				config.getCacheLocation());
	}
	 ...
	 * @throws Exception
	 */
	public static FedXSailRepository initializeFederation(Config config, EndpointListProvider endpointListProvider)
			throws FedXException {
//		FedXSailRepository ff = null;
	//	ForkJoinPool fjp = new ForkJoinPool();
	//	fjp.submit(()->
		//{	
			FedXSailRepository ifs =  initializeFederation(config, endpointListProvider, config.getSummaryProvider(),config.getCacheLocation());
	 ...
	 * @return
	 */
	private static FedXSailRepository initializeFederation(Config config, EndpointListProvider endpointListProvider,
			SummaryProvider summaryProvider, String cacheLocation) throws FedXException {
		System.out.println("This is timing in initializeFederation:"+LocalTime.now());
//		ForkJoinPool fjp = new ForkJoinPool();
		
			Cache cache = //fjp.submit(()->
			new MemoryCache(cacheLocation);//).join();
	//		fjp.shutdown();
//ForkJoinPool fjp = new ForkJoinPool();
//fjp.submit(()->cache.initialize());	
//fjp.shutdown();
...
//ForkJoinPool fjp4 =// new ForkJoinPool();
		FedX federation =// fjp4.submit(()-> 
		new FedX(config, cache,endpointListProvider, summaryProvider);//).join();
//fjp4.shutdown();
//ForkJoinPool fjp5 = new ForkJoinPool();
	FedXSailRepository repo =
				//fjp5.submit(()->
		new FedXSailRepository(federation);//).join();
	//		fjp5.shutdown();	
		try {
			

		//	ForkJoinPool fjp1 = new ForkJoinPool();
		//	Stream.of(fjp1.submit(()->
			repo.initialize();//));	
			//fjp1.shutdown();
