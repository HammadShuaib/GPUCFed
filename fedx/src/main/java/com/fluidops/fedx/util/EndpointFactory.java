...
/**
 * Utility class providing various methods to create Endpoints to be used as
 * federation members.
 * 
...
public class EndpointFactory {
	 ...
	 * @param endpoint the URL of the SPARQL endpoint, e.g.
	 *                 http://dbpedia.org/sparql
	 * 
	 * @return an initialized {@link Endpoint} containing the repository
	 * 
	 * @throws Exception
	 */
	static String id;
	public static Endpoint loadSPARQLEndpoint(Config config, HttpClient httpClient, String name, String endpoint)
	...
	/**
	 * Construct a SPARQL endpoint using the the provided information and the host
	 * of the url as name.
	 * 
	 * @param endpoint the URL of the SPARQL endpoint, e.g.
	 *                 http://dbpedia.org/sparql
	 ...
	 * @throws FedXException
	 */
	public static Endpoint loadSPARQLEndpoint(Config config, HttpClient httpClient, String endpoint)
			throws FedXException {
		try {
			id = new URL(endpoint).getHost();
			if (id.equals("localhost")) {
				id = id + "_" + new URL(endpoint).getPort();
			}
		//	ForkJoinPool fjp = new ForkJoinPool(6);
			return //fjp.submit(()->
			loadSPARQLEndpoint(config, httpClient, "http://" + id, endpoint);//).join();
		} catch (MalformedURLException e) {
			throw new FedXException("Malformed URL: " + endpoint);
		}
	}

	public static Endpoint loadRemoteRepository(Config config, String repositoryServer, String repositoryName)
			throws FedXException {
		EndpointProvider repProvider = new RemoteRepositoryProvider(config);
		return repProvider
				.loadEndpoint(new RemoteRepositoryGraphRepositoryInformation(repositoryServer, repositoryName));
	}
	 ...
	 * @throws Exception
	 */
	public static Endpoint loadNativeEndpoint(Config config, String name, String location, File baseDir)
			throws FedXException {
		return loadNativeEndpoint(config, name, baseDir.getAbsolutePath() + "/" + location);
	}

	/**
	 * Construct a NativeStore endpoint using the provided information and the file
	 * location as name. Note: "http://" is prepended to use standard names
	 * 
	 ...
	 * {@link NativeGraphRepositoryInformation} and
	 * {@link SPARQLGraphRepositoryInformation}.
	 * 
	 * @param dataConfig
	 * 
	 ...
	 * @throws Exception
	 */
	public static List<Endpoint> loadFederationMembers(Config config, HttpClient httpClient, File dataConfig)
			throws FedXException {
		...
		for (Statement st : graph.filter(null,
				SimpleValueFactory.getInstance().createIRI("http://fluidops.org/config#store"), null)) {
	...
	public static Endpoint loadEndpoint(Config config, HttpClient httpClient, Model graph, Resource repNode,
			Value repType) throws FedXException {
